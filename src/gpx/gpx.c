...
{
//    SHOW(VERBOSE( fprintf(gpx->log, "write_8 int value = %d" EOL, value) ));
...
//        VERBOSE( fprintf(gpx->log, "Calculated crc" EOL) );
    }
    size_t length = gpx->buffer.ptr - gpx->buffer.out;
    gpx->accumulated.bytes += length;
//    SHOW(VERBOSE( fprintf(gpx->log, "length = %d" EOL, length)));
//    SHOW(VERBOSE( fprintf(gpx->log, "hex buffer.out =")));
//    for(int i=0; i<length; i++) {
//        SHOW(VERBOSE( fprintf(gpx->log, " %X", gpx->buffer.out[i])));
//    }
//    SHOW(VERBOSE( fprintf(gpx->log, "" EOL)));
...
static int get_version(Gpx *gpx)
{
    ...
    SHOW(VERBOSE( fprintf(gpx->log, "Get version number" EOL) ));
...
static int initialize_firmware(Gpx *gpx)
{
    ...
    SHOW(VERBOSE( fprintf(gpx->log, "Initialize firmware" EOL) ));
    return end_frame(gpx);
}
...
static int clear_buffer(Gpx *gpx)
{
    ...
    SHOW(VERBOSE( fprintf(gpx->log, "Clear buffer" EOL) ));
    write_8(gpx, 3);
...
static int set_valve(Gpx *gpx, unsigned extruder_id, unsigned state)
{
    ...
    if(gpx->machine.type >= MACHINE_TYPE_REPLICATOR_1) {
        ...
        SHOW(VERBOSE( fprintf(gpx->log, "set valve" EOL)))
        write_8(gpx, 136);
...
//    SHOW(VERBOSE( fprintf(gpx->log, "Am I here?" EOL) ));
    write_8(gpx, 150);
...
//        write_8(gpx, gpx->flag.relativeCoordinates);
...
static int parse_macro(Gpx *gpx, const char* macro, char *p)
{
    ...
    VERBOSE( fprintf(gpx->log, "I is using macro?" EOL) );
    while(*p != 0) {
...
{
    VERBOSE( fprintf(gpx->log, "register_callback" EOL) );
    ...
    if(gpx->command.flag & G_IS_SET) {
        ...
        switch(gpx->command.m) {
            ...
            case 2:
                if(program_is_running()) {
                    end_program();
                    SHOW(VERBOSE( fprintf(gpx->log, "HERE 1" EOL) ));
...
//                        CALL( change_extruder_offset(gpx, gpx->current.extruder) );
                        ...
                        if(percent == 100) {
                            ...
                            gpx->flag.macrosEnabled = 0;
                            end_program();
                            SHOW(VERBOSE( fprintf(gpx->log, "HERE 2" EOL) ));
                            CALL( set_build_progress(gpx, 100) );
                            CALL( end_build(gpx) );
...
//                            }
                            if(percent == 1 || gpx->total.time == 0.0 || gpx->flag.buildProgress == 0) {
                                CALL( set_build_progress(gpx, percent) );
                                gpx->current.percent = percent;
                            }
                        }
            ...
            case 106: {
                ...
                if(!gpx->flag.reprapFlavor && gpx->machine.type >= MACHINE_TYPE_REPLICATOR_1) {
                    if(gpx->flag.dittoPrinting) {
                        ...
                        SHOW(VERBOSE( fprintf(gpx->log, "Switching fan state to %d" EOL, state)))
            ...
            case 137:
                if(program_is_running()) {
                    ...
                    gpx->flag.macrosEnabled = 0;
                    end_program();
                    SHOW(VERBOSE( fprintf(gpx->log, "HERE 3" EOL) ));
                    CALL( set_build_progress(gpx, 100) );
                    CALL( end_build(gpx) );
                    gpx->current.percent = 100;
                }
    ...
    else if(gpx->command.flag & T_IS_SET) {
        gpx->target.extruder = gpx->command.t;
        if(!gpx->flag.dittoPrinting) {
            int timeout = gpx->command.flag & P_IS_SET ? (int)gpx->command.p : MAX_TIMEOUT;
            CALL( do_tool_change(gpx, timeout) );
            command_emitted++;
...
//            }
        ...
        if(program_is_running()) {
            end_program();
            SHOW(VERBOSE( fprintf(gpx->log, "HERE 4" EOL) ));
            CALL( set_build_progress(gpx, 100) );
            CALL( end_build(gpx) );
        }
...
static int port_handler(Gpx *gpx, Sio *sio, char *buffer, size_t length)
{
    ...
    if(length) {
        ...
        int retry_count = 0;
        do {
//            SHOW(VERBOSE( fprintf(gpx->log, "hex buffer =") ));
//            for(int i=0;i<length;i++){
//                SHOW(VERBOSE( fprintf(gpx->log, " %X", buffer[i])))
//            }
//            SHOW(VERBOSE( fprintf(gpx->log, "" EOL)))
            SHOW(VERBOSE( fprintf(gpx->log, "Sending %X" EOL, buffer[2])));
            
            // send the packet
            if((bytes = write(sio->port, buffer, length)) == -1) {
                SHOW(VERBOSE( fprintf(gpx->log, "errno = %d" EOL, errno) ));
                return errno;
            }
            else if(bytes != length) {
                SHOW(VERBOSE( fprintf(gpx->log, "ESIOWRITE = %d" EOL, ESIOWRITE) ));
                return ESIOWRITE;
            }
            sio->bytes_out += length;
//            SHOW(VERBOSE( fprintf(gpx->log, "Wrote to the serial port" EOL) ));

            if(sio->bytes_in) {
                VERBOSE( fprintf(gpx->log, "recieving the response" EOL) );
                // recieve the response
                if((bytes = read(sio->port, gpx->buffer.in, 2)) == -1) {
                    return errno;
                }
                else if(bytes != 2) {
                    SHOW(VERBOSE( fprintf(gpx->log, "ESIOREAD = %d" EOL, ESIOREAD) ));
                    return ESIOREAD;
                }
                // invalid start byte
                if(gpx->buffer.in[0] != 0xD5) {
                    SHOW(VERBOSE( fprintf(gpx->log, "ESIOFRAME = %d" EOL, ESIOFRAME) ));
                    return ESIOFRAME;
                }
            }
            else {
//                VERBOSE( fprintf(gpx->log, "I've decided to read first" EOL) );
                // first read
                for(;;) {
                    // read start byte
                    if((bytes = read(sio->port, gpx->buffer.in, 1)) == -1) {
                        SHOW(VERBOSE( fprintf(gpx->log, "read start byte errno" EOL) ));
                        return errno;
                    }
                    else if(bytes != 1) {
                        SHOW(VERBOSE( fprintf(gpx->log, "read start byte ESIOREAD" EOL) ));
                        return ESIOREAD;
                    }
                    // loop until we get a valid start byte
                    if(gpx->buffer.in[0] == 0xFFFFFFD5) break;
                }
                // read length
//                SHOW(VERBOSE( fprintf(gpx->log, "READ LENGTH" EOL) ));
                if((bytes = read(sio->port, gpx->buffer.in + 1, 1)) == -1) {
                    SHOW(VERBOSE( fprintf(gpx->log, "errno*" EOL) ));
                    return errno;
                }
                else if(bytes != 1) {
                    SHOW(VERBOSE( fprintf(gpx->log, "ESIOREAD*" EOL) ));
                    return ESIOREAD;                    
                }
//                SHOW(VERBOSE( fprintf(gpx->log, "I READ THE LENGTH" EOL) ));
            }
            size_t payload_length = gpx->buffer.in[1];
            // recieve payload
//            SHOW(VERBOSE( fprintf(gpx->log, "RECIEVING PAYLOAD" EOL) ));
            ...
            // check CRC
//            SHOW(VERBOSE( fprintf(gpx->log, "Checking CRC" EOL) ));
            ...
            if(crc != calculate_crc((unsigned char*)gpx->buffer.in + 2, payload_length)) {
                SHOW(VERBOSE( fprintf(gpx->log, "CRC FAILURE" EOL)));
                fprintf(gpx->log, "(retry %u) Input CRC mismatch: packet discarded" EOL, retry_count);
                rval = ESIOCRC;
                goto L_RETRY;
            }
            // check response code
            SHOW(VERBOSE( fprintf(gpx->log, "response code %X" EOL, gpx->buffer.in[2])))
            ...
            switch((unsigned)gpx->buffer.in[2]) {
                ...
                case 0xFFFFFF82:
                    do {
                        // wait for 1/10 seconds EDIT - 
                        usleep(100000);
                        // query buffer size
                        buffer_size_query[3] = calculate_crc((unsigned char *)buffer_size_query + 2, 1);
                        SHOW(VERBOSE( fprintf(gpx->log, "buffer_size_query[3] = %x" EOL, buffer_size_query[3])));
                        VERBOSE( fprintf(gpx->log, "(Calling port_handler" EOL) );
...
L_RETRY:
        ...
        SHOW(VERBOSE( fprintf(gpx->log, "I give up" EOL) ));
    }
...
int gpx_convert_and_send(Gpx *gpx, FILE *file_in, int sio_port)
{
    ...
    if(file_in && file_in != stdin) {
        ...
        i = 1;
        gpx->flag.framingEnabled = 1;
        SHOW(VERBOSE( fprintf(gpx->log, "Calling port_handler in single-pass" EOL) ));
    ...
    for(;;) {
        ...
        if(program_is_running()) {
            end_program();
            SHOW(VERBOSE( fprintf(gpx->log, "HERE 5" EOL) ));
            CALL( set_build_progress(gpx, 100) );
            CALL( end_build(gpx) );
        }
        ...
        SHOW(VERBOSE( fprintf(gpx->log, "I thought we got here" EOL) ));
    ...
    gpx->flag.logMessages = logMessages;;
    SHOW(VERBOSE( fprintf(gpx->log, "success %d?" EOL, SUCCESS) ));
    return SUCCESS;
}
